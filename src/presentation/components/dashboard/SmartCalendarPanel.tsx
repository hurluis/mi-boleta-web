...
export function SmartCalendarPanel({ upcomingTickets }: SmartCalendarPanelProps) {
  ...
  return (
    <section className="rounded-2xl border border-soft bg-surface shadow-card" aria-label="Calendario inteligente">
      ...
      <div className="grid gap-4 p-5 md:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-xl border border-soft bg-elevated p-4">
          ...
          {isLoading ? (
            <div className="mt-4 h-20 animate-pulse rounded-xl bg-muted/10" />
          ) : isError || !nextHoliday ? (
        ...
        <div className="rounded-xl border border-soft bg-elevated p-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="rounded-xl bg-brand-500/10 px-3 py-2 font-display text-2xl font-bold text-brand-700 dark:text-brand-200">
          ...
          <div className="mt-4 space-y-2">
            {ticketsOnHoliday.slice(0, 3).map((ticket) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center justify-between gap-3 rounded-xl border border-soft bg-surface px-3 py-2 ring-focus transition hover:border-brand-400/40"
                >
            ...
            {ticketsOnHoliday.length === 0 && (
              <div className="rounded-xl border border-dashed border-soft px-3 py-4 text-sm text-muted">
                Los proximos festivos apareceran aqui como referencia para planear tus revisiones.
              </div>
            )}
          </div>
        </div>
